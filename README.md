...
- 🌟 **双引擎识别**：集成 OpenCV QRCode 和 WeChatQRCode，识别率高达 100%
- ⚡ **并行处理**：支持多进程并行扫描（默认 8 进程）
